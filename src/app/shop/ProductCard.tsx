...
const ProductGrid: React.FC = () => {
  ...
  const products: Product[] = [
    ...
    // (Other products as in your original data)...
  ];
  ...
  const PaginationItem: React.FC<PaginationItemProps> = ({
    ...
    const baseClasses =
      "px-6 rounded-xl h-[60px] w-[60px] max-md:px-5 cursor-pointer transform transition-all duration-300";
  ...
  const ProductItem: React.FC<Product> = ({ price, title, imageUrl }) => {
    return (
      <div className="relative group flex flex-col items-start text-black mb-8 max-md:mb-5 transform transition-transform duration-300 hover:scale-105 hover:shadow-lg p-4 bg-white rounded-lg">
        <img
          loading="lazy"
          src={imageUrl}
          alt={title}
          className="object-cover w-full aspect-square rounded transition-all duration-300 group-hover:opacity-90"
        />
        <div className="mt-4 text-base font-semibold group-hover:text-gray-700">
          {title}
        </div>
        <div className="mt-3 text-lg font-medium text-gray-600 group-hover:text-gray-900">
          {price}
        </div>
      </div>
    );
    ...
    <div className="flex flex-col items-center w-full max-w-[1242px] px-4 mx-auto">
      ...
      <div className="flex flex-wrap gap-5 justify-start w-full">
        {products.map((product) => (
          <div key={product.id} className="w-full sm:w-[48%] md:w-[30%] p-4">
      ...
      <div className="flex gap-5 justify-center mt-10 max-md:mt-5 w-full">
        <div className="px-6 py-3 rounded-xl h-[60px] w-[60px] bg-amber-100 flex items-center justify-center">
          1
        </div>
        <div className="px-6 py-3 rounded-xl h-[60px] w-[60px] bg-yellow-50 flex items-center justify-center">
          2
        </div>
        <div className="px-6 py-3 rounded-xl h-[60px] w-[60px] bg-yellow-50 flex items-center justify-center">
          3
        </div>
        <div className="px-7 py-4 font-light bg-yellow-50 rounded-xl max-md:px-5 cursor-pointer hover:bg-yellow-200 transform transition-all duration-300">
          Next
        </div>
      </div>
    </div>
